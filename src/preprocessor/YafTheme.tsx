import { DefaultTheme, JSX, PageEvent, Reflection, Renderer, RendererEvent } from "typedoc";
...
export class YafTheme extends DefaultTheme {
	...
	constructor(renderer: Renderer) {
		...
		this.listenTo(this.owner, RendererEvent.END, () => {
			...
			const rootSubPaths = fs.readdirSync(outDir)
				.filter(file => fs.statSync(path.join(outDir, file)).isDirectory());
			this._contextCache?.browserDataCache.setCacheItem('yaf.rootSubPaths', rootSubPaths)
	...
	override render(page: PageEvent<Reflection>): string {
		const templateOutput = (page.url === 'index.html')?
